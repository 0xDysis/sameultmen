fluid = {
	...
	initVectorField:function(){
        ...
        this.vectorField = this.isMobile ?
            new vectorField( window.innerWidth, window.innerHeight, 60, 30 ):
            new vectorField( window.innerWidth, window.innerHeight, 120, 60 );
    },
	...
	updateParticles:function(){
		...
		var maxspeedd = this.maxParticleSpeedd;
		var dis,mf;
	...
	drawParticles:function(){
		...
		var cols = this.colors;
		var mf;
		var imaxspeedd = 1 / this.maxParticleSpeedd;

		var p, i = this.particleCount;
		while( i-- ){
			p = this.particles[ i ];

			mf = p.speedd * imaxspeedd;
			if( mf > 1 ) mf = 1;
